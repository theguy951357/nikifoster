...
(function () {
  ...
  var db, auth;
  var extraCards = {}; /* tracks extra cards per section */

  /* ── Defaults — used to pre-populate form when database is empty ──── */
  window.CMS_DEFAULTS = {
  ...
  /* Call init once DOM is ready — Firebase scripts load synchronously
     so they are available as soon as the DOM is parsed */
  ...
  /* ── Populate form — always falls back to defaults ────────────────── */
  function populateForm(data) {
    ...
    /* Lists */
    var assistList   = (data.assist_list && data.assist_list.length)         ? data.assist_list         : D.assist_list;
    var practiceList = (data.practice_areas_list && data.practice_areas_list.length) ? data.practice_areas_list : D.practice_areas_list;
    var intakeList   = (data.intake_list && data.intake_list.length)         ? data.intake_list         : D.intake_list;

    renderListEditor('assist-list-editor',   assistList);
    renderListEditor('practice-list-editor', practiceList);
    renderListEditor('intake-list-editor',   intakeList);

    /* Extra cards */
    extraCards = {};
    var sections = ['services','how_it_works','about','book','contact'];
    sections.forEach(function (section) {
      ...
      if (data.extra_cards && data.extra_cards[section]) {
        var cards = Object.values(data.extra_cards[section]);
  ...
  /* ── Extra card admin UI ──────────────────────────────────────────── */
  function addExtraCard(section) {
    renderExtraCardAdmin(section, '', '');
  }
  ...
  /* ── Save — uses update() to never blank other fields ────────────── */
  function saveContent() {
    ...
    /* Extra cards — convert arrays to objects for Firebase */
    newData.extra_cards = {};
    ...
    /* update() merges with existing data — never blanks other fields */
